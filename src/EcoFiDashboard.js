...
const EcoFiDashboard = () => {
  ...
  // Recalculate environmental impact whenever cumulativeKwh changes
  useEffect(() => {
    console.log("useEffect triggered - cumulativeKwh:", cumulativeKwh);
    
    // Calculate environmental impact directly here to avoid dependency issues
    ...
    const impactData = {
      co2Reduced: Math.round((cumulativeKwh || 0) * co2PerKwh),
      treesPlanted: Math.round((cumulativeKwh || 0) * treesPerKwh),
      energySaved: cumulativeKwh || 0,
      waterConserved: Math.round((cumulativeKwh || 0) * waterPerKwh)
    };
    
    console.log("Calculated environmental impact:", impactData);
    setEnvironmentalImpact(impactData);
    
    // Also update impact score based on milestones
    if (milestones.length > 0 && cumulativeKwh > 0) {
      const lastMilestone = milestones[milestones.length - 1];
      const maxThreshold = lastMilestone.threshold;
      const score = Math.min((cumulativeKwh / maxThreshold) * 100, 100);
      setImpactScore(Math.round(score));
      console.log("Updated impact score:", Math.round(score));
    }
  }, [cumulativeKwh, milestones]); // Removed updateEnvironmentalImpact dependency

  // Toast helper functions (defined early to avoid initialization order issues)
  ...
  // Debug function to test oracle connectivity  
  const testOracleConnection = useCallback(async () => {
    if (!provider) {
      console.log("No provider available for oracle test");
    ...
    try {
      ...
      console.log("Oracle contract address:", oracle.target || oracle.address);
      ...
      console.log("- Cumulative CO2:", co2.toString());
      
      // If we have data, update it immediately
      const kwhValue = Number(kwh.toString());
      console.log("Converting kWh:", kwh.toString(), "->", kwhValue);
      
      if (kwhValue !== cumulativeKwh) {
        console.log("Updating cumulativeKwh from", cumulativeKwh, "to", kwhValue);
        setCumulativeKwh(kwhValue);
        showToast(`Oracle data updated: ${kwhValue} kWh`, "success");
      } else {
        console.log("kWh value unchanged:", kwhValue);
      }
      
      console.log("=== ORACLE TEST COMPLETE ===");
    } catch (error) {
      console.error("Oracle test failed:", error);
      showToast(`Oracle test failed: ${error.message}`, "error");
    }
  }, [provider, cumulativeKwh, showToast]);

  // Initialize environmental impact on component mount
  useEffect(() => {
    console.log("Component mounted, initializing environmental impact");
    // Initialize with 0 values directly to avoid circular dependency
    const impactData = {
      co2Reduced: 0,
      treesPlanted: 0,
      energySaved: 0,
  ...
  // Auto-refresh data for all users (especially investors) 
  useEffect(() => {
    if (!provider || !walletAddress) return;
    
    // Test oracle connection immediately
    testOracleConnection();
    
    const refreshInterval = setInterval(() => {
      console.log("Auto-refreshing contract data for real-time updates");
      fetchContractData(provider, walletAddress);
      testOracleConnection(); // Also test oracle on each refresh
    }, 15000); // Refresh every 15 seconds
    
    return () => clearInterval(refreshInterval);
  }, [provider, walletAddress, testOracleConnection]);

  // Helper function to update environmental impact
  const updateEnvironmentalImpact = useCallback((kwhValue) => {
    console.log("updateEnvironmentalImpact called with kWh:", kwhValue);
    ...
    // Calculate impact even if kwhValue is 0 (to show 0 values)
    const impactData = {
      co2Reduced: Math.round((kwhValue || 0) * co2PerKwh),
      treesPlanted: Math.round((kwhValue || 0) * treesPerKwh),
      energySaved: kwhValue || 0,
      waterConserved: Math.round((kwhValue || 0) * waterPerKwh)
    };
  ...
  const fetchContractData = async (providerInstance, address) => {
    ...
    try {
      ...
      try {
        console.log("Attempting to fetch cumulativeKwh from oracle...");
        console.log("Oracle contract address:", oracle.target || oracle.address);
        
        const kwh = await oracle.cumulativeKwh();
        console.log("Raw kWh value from contract:", kwh.toString());
        
        // Fix: The oracle stores kWh as regular uint256, not in wei format
        const kwhValue = Number(kwh.toString()); // Direct conversion from BigInt to Number
        setCumulativeKwh(kwhValue);
        console.log("Fetched cumulative kWh:", kwhValue);
        
        // Force immediate update of environmental impact
        if (kwhValue > 0) {
          console.log("Immediately updating environmental impact with new kWh value");
          updateEnvironmentalImpact(kwhValue);
        }
      } catch (error) {
        console.error("Failed to fetch cumulativeKwh:", error);
        console.error("Oracle contract details:", oracle);
      }
      ...
      // Environmental impact will be calculated automatically via useEffect when cumulativeKwh changes
  ...
  const handlePushImpactData = async () => {
    ...
    try {
      ...
      // Wait a moment for transaction to be mined, then refresh with a longer timeout
      setTimeout(() => {
        console.log("Refreshing contract data after impact update");
        fetchContractData(provider, walletAddress);
      }, 5000);
    } catch (error) {
...
<body>
  ...
  const OracleSimulator = () => (
    <div className="glass-card p-6 rounded-xl border border-purple-500/20">
      ...
      {/* Debug: Test Oracle Connection */}
      <div className="mt-4 pt-4 border-t border-gray-600">
        <button
          onClick={testOracleConnection}
          className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm transition-all"
        >
        ...
        <button
          onClick={async () => {
            setDeltaKwh('1000000');
            setDeltaCO2('400000');
            setTimeout(() => handlePushImpactData(), 500);
          }}
          className="w-full mt-2 bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded-lg text-sm transition-all"
        >
          ⚡ Quick Test: Push 1M kWh
        </button>
      </div>
    </div>
  );
              ...
              <div className="space-y-6">
                ...
                <div className="glass-card p-4 rounded-xl">
                  ...
                  <div className="text-xs text-gray-400 space-y-1">
                    <div>Cumulative kWh: {cumulativeKwh}</div>
                    <div>CO2 Reduced: {environmentalImpact.co2Reduced} kg</div>
                    <div>Trees Planted: {environmentalImpact.treesPlanted}</div>
                    <div>Water Conserved: {environmentalImpact.waterConserved} L</div>
                  </div>
                </div>
